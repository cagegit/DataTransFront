...
let err = require('../main/error');
let pub = require('../common/public');

describe('错误相关接口测试', function () {
    let group = global.group || '';
    ...
    it('获取全部错误信息', function (done) {
        ...
        xmlParserAttr.parseString(xml, function (error, result) {
            if (error) {
                ...
                err.query_apply_full_error(result.dip_command.command_data, res).then(function (result) {
    ...
    it('获取当前错误信息', function (done) {
        let xml = `<dip_command><command>query_apply_current_error</command><command_data><group>${group}</group><component_name>${apply}</component_name></command_data></dip_command>`;

        xmlParserAttr.parseString(xml, function (error, result) {
            if (error) {
                console.error(error);
            } else {
                err.query_apply_current_error(result.dip_command.command_data, res).then(function (result) {

                    console.log('====', result);
                    expect(result).to.be.an('object');
if (!pub.isEmptyObj(result)){
    ...
    it('标识错误信息', function (done) {
        let xml = `<dip_command><command>marked_apply_error</command><command_data><group>${group}</group><component_name>${apply}</component_name><location><offset></offset></location></command_data></dip_command>`;

        xmlParserAttr.parseString(xml, function (error, result) {
            if (error) {
                console.error(error);
            } else {
                err.marked_apply_error(result.dip_command.command_data, res).then(function (result) {
    ...
    it('导出错误信息到excel表格', function (done) {
        let xml = `<dip_command><command>download_error_file</command><command_data><group>${group}</group><component_name>${apply}</component_name></command_data></dip_command>`;

        xmlParserAttr.parseString(xml, function (error, result) {
            if (error) {
                console.error(error);
            } else {
                err.download_error_file(result.dip_command.command_data, res).then(function (result) {
    ...
    it('导出错误信息到.gz', function () {
        ...
        return err.exp_err(req, res).then(function (result) {
            console.log('====', result);
            expect(result).to.be.an('object');
